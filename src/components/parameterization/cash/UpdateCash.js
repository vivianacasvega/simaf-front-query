...
import {
 ...
 const UpdateCash = (props) => {
  ...
  const returnCancel = async () => {
    const resc = await API.ExtCash.getreturncancel(getItem("userId"));
  ...
  const returnSign = async (us, est, doctype) => {
    try{
      const ressign = await API.ExtCash.returnsign({
        'us': us,
        'est': est,
        'doctype': doctype
      })
  ...
  const setMessageSuccess = (messageSuccess) => {
    ...
    try{
      const resCancel = await API.ExtCash.update({
        'year': props.year,
        'doc': props.doctype,
        'sec': props.sec,
  ...
  const updateAprov = async () => {
    try{
      const resAprov = await API.ExtCash.update({
        'year': props.year,
        'doc': props.doctype,
        'sec': props.sec,
        'number': (String(props.number)),
