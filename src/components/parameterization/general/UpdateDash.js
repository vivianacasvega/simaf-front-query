...
import {
 ...
 const UpdateDash = (props) => {
  ...
  const returnCancel = async () => {
    const resc = await API.ExtGeneralDash.getreturncancel(getItem("userId"));
  ...
  const returnSign = async (us, est, doctype) => {
    try{
      const ressign = await API.ExtGeneralDash.returnsign({
        'us': us,
        'est': est,
        'doctype': doctype
      })
  ...
  const updateCancel = async () => {
    ...
    try{
      const resCancel = await API.ExtGeneralDash.update({
        'year': props.year,
        'doc': props.doctype,
        'sec': props.sec,
