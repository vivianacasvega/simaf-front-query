...
import API from '../../../api';
import UpdateDash from './UpdateDash';
...
const DocDash = (props) => {
  ...
  const getDoc = async (year, doc, sec, number) => {
    try{
      setMostrar(false);
      const res = await API.ExtGeneralDash.getdoc({
        year: year,
        doc: doc,
        sec: sec,
        number: number
      ...
      if(res.ok){
        console.log(res.ok);
        //console.log(res.data["Entity"]["DocumentKey"]["Number"]);
        //console.log(res.data["Entity"]["Status"].Code);
